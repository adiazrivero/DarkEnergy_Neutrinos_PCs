...
if dset == 5: #all = WITH DES WL
    label = 'L5'
    print('weak lensing likelihood included')

elif dset == 1: #reduced = WITHOUT DES WL
    label = 'L1'
...
"""if label == 'L5':
    ...
    shearps_dir = '/n/dvorkin_lab/anadr/Neutrinos_DDE/DES_WPC135/shear_PS/LCDM/'
    ...
    for num_file in range(ini,fin):
        ...
        f = open(shearps_dir + 'shearPS%s_n%s_%s.txt' % (label,which_bin,num_file),'r')
    ...
    for i,j in zip(arrs,labs):
	    ...
	    a = np.percentile(i,perc,axis=0)
            ...
            file1 = open('/n/home04/adiazrivero/Neutrinos_DDE/Results/CLs/DES_WPC135/Arrays_135/LCDM/%s%s_n%s_%s.txt' % (j,label,which_bin,perc), 'w')
            for item in a:
                file1.write('%s\n' % item)
            file1.close()
"""
...
for arr,lab in zip(arrs,labs):
	...
	a = np.percentile(arr,perc,axis=0)
        ...
        file1 = open('/n/home04/adiazrivero/Neutrinos_DDE/Results/CLs/DES_WPC135/Arrays_135/LCDM/%s_%s_%s.txt' % (lab,perc,label), 'w')
